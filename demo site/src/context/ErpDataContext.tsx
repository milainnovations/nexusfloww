...
export const ErpDataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [students, setStudents] = useState<Student[]>(() => {
    const saved = localStorage.getItem('bluke_students_school_v2')
    return saved ? JSON.parse(saved) : INITIAL_STUDENTS
  })

  const [facultyList, setFacultyList] = useState<Faculty[]>(() => {
    const saved = localStorage.getItem('bluke_faculty_school_v2')
    return saved ? JSON.parse(saved) : INITIAL_FACULTY
  })

  const [timetableSlots, setTimetableSlots] = useState<TimetableSlot[]>(() => {
    const saved = localStorage.getItem('bluke_timetable_school_v2')
    return saved ? JSON.parse(saved) : INITIAL_TIMETABLE
  })

  const [courses, setCourses] = useState<Course[]>(() => {
    const saved = localStorage.getItem('bluke_courses_school_v2')
    return saved ? JSON.parse(saved) : INITIAL_COURSES
  })

  const [exams, setExams] = useState<Exam[]>(() => {
    const saved = localStorage.getItem('bluke_exams_school_v2')
    return saved ? JSON.parse(saved) : INITIAL_EXAMS
  })

  const [invoices, setInvoices] = useState<Invoice[]>(() => {
    const saved = localStorage.getItem('bluke_invoices_school_v2')
  ...
  const [hostelRooms, setHostelRooms] = useState<HostelRoom[]>(() => {
    const saved = localStorage.getItem('bluke_hostel_school_v2')
    return saved ? JSON.parse(saved) : INITIAL_HOSTEL
  })

  const [libraryBooks, setLibraryBooks] = useState<LibraryBook[]>(() => {
    const saved = localStorage.getItem('bluke_library_school_v2')
    return saved ? JSON.parse(saved) : INITIAL_LIBRARY_BOOKS
  })

  const [usersList, setUsersList] = useState<UserRole[]>(() => {
    const saved = localStorage.getItem('bluke_users_school_v2')
    return saved ? JSON.parse(saved) : INITIAL_USERS
  })

  // Sync with localStorage
  useEffect(() => {
    localStorage.setItem('bluke_students_school_v2', JSON.stringify(students))
  }, [students])

  useEffect(() => {
    localStorage.setItem('bluke_faculty_school_v2', JSON.stringify(facultyList))
  }, [facultyList])

  useEffect(() => {
    localStorage.setItem('bluke_timetable_school_v2', JSON.stringify(timetableSlots))
  }, [timetableSlots])

  useEffect(() => {
    localStorage.setItem('bluke_courses_school_v2', JSON.stringify(courses))
  }, [courses])

  useEffect(() => {
    localStorage.setItem('bluke_exams_school_v2', JSON.stringify(exams))
  }, [exams])

  useEffect(() => {
    localStorage.setItem('bluke_invoices_school_v2', JSON.stringify(invoices))
  }, [invoices])
