...
export const ExamsGradesPage: React.FC = () => {
  const { user } = useAuth()
  const { students, exams, addExam, updateExamMarks } = useErpData()
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <Modal
        ...
        subtitle={selectedStudentForCard ? `${selectedStudentForCard.name} (${selectedStudentForCard.classGrade})` : ''}
      >
        {selectedStudentForCard && (
          <div className="space-y-4 text-xs">
            <div className="rounded-xl border border-[#c5d8cd] bg-[#f8faf9] p-4 text-center">
              <h3 className="font-editorial text-lg font-bold text-[#0e4b38]">
                Greenwood International School
              </h3>
              <p className="text-[10px] text-[#71877e]">Affiliation #83042 • Term 1 Assessment Report</p>
            </div>

            <div className="grid grid-cols-2 gap-2 text-xs border-b border-[#edf3ef] pb-3">
              <div>
                <span className="text-[#71877e]">Student: </span>
                <span className="font-bold text-[#14241e]">{selectedStudentForCard.name}</span>
              </div>
              <div>
                <span className="text-[#71877e]">Roll #: </span>
                <span className="font-mono font-bold">{selectedStudentForCard.rollNumber}</span>
              </div>
              <div>
                <span className="text-[#71877e]">Class: </span>
                <span className="font-semibold">{selectedStudentForCard.classGrade}</span>
              </div>
              <div>
                <span className="text-[#71877e]">Attendance: </span>
                <span className="font-bold text-[#0e4b38]">{selectedStudentForCard.attendancePct}%</span>
              </div>
            </div>

            <div className="rounded-lg bg-[#f0f7f3] p-3 text-xs">
              <span className="font-bold text-[#0e4b38]">Term 1 Overall Score: </span>
              <span className="font-bold text-[#14241e]">
                {selectedStudentForCard.termPercentage}% (Grade {selectedStudentForCard.overallGrade})
              </span>
            </div>

            <div className="rounded-lg border border-[#edf3ef] p-3 text-xs italic text-[#50685e]">
              "{selectedStudentForCard.remarks}"
            </div>
